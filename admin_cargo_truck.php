...
?>
  ...
  if (isset($_POST["delete"]))
  {
     ...
     $dquery=$dbconnection->prepare("DELETE FROM cargo_truck WHERE truckno='$d_truckno'");
     $dquery->execute();
          ...
          <div class="col-md-12">
           ...
           <div class="bottom_work" style="background-color:#E7EEF4;">
                  <div class="header_part d-flex flex-row justify-content-between" style="border-bottom:2px solid black; padding:20px">
                    <h5>Manage Cargo Truck Information</h5>
                    <form method="post" action="admin_cargo_truck.php">
                    ...
                    <div class="display-info" id="table_part">
                        <?php
                            if (isset($_POST['addbtn']) || isset($_POST['edit']) || isset($_POST['search_bt']) )  {
                                ...
                                {   
                                    //$next_truckno = getNextTruckNo(); // Assuming there's a function to get the next available customer ID
                                    add_form();
                                }
                                else if(isset($_POST['edit']))
                                {
                                    $e_truckno = $_POST['action_id'];
                                    //echo $e_costid;
                                    $edit_row_query= "SELECT t.truckno, t.max_weight FROM cargo_truck t WHERE t.truckno= '$e_truckno'";
                                    $erow=$dbconnection->query($edit_row_query);
                                    $etruckno=$e_truckno;
                                    while($edit_row = $erow->fetch_assoc())
                                    {
                                      $etruckno=$edit_row["truckno"];
                                      $emax_weight=$edit_row["max_weight"];
                                     
                                      show_edit_form($etruckno,$emax_weight);
                                    } 
                                }
                                else if(isset($_POST['search_bt']))
                                {
                                  $search = $_POST['search'];
                                  $sql = "SELECT t.truckno, t.max_weight FROM cargo_truck t WHERE t.truckno LIKE '%$search%' OR t.max_weight LIKE '%$search%'  ";
                            ...
                            if (isset($_POST["add_btn"]))
                            {
                                ...
                                $ntruckno=$_POST["ntruckno"];
                                $nmaxw=$_POST["nmaxw"];
                                
                                try {
                                    // Check if the truckno exists in the table
                                    $check_query = 'SELECT * FROM cargo_truck WHERE truckno = :truckno';
                                    $check_sta=$conn->prepare($check_query);
                                    $check_sta-> bindValue( ":truckno", $ntruckno, PDO::PARAM_STR);
                                    $check_sta->execute();
                                
                                    if ($check_sta->rowCount() > 0) {
                                        echo "<script>window.alert('The tuck no already exists in the table. Please enter a different tuck no.'); </script>";
                                        show_add_again($ntruckno,$nmaxw);                                    
                                    } else {
                                        //Prepare the INSERT statement
                                        $add_query = 'INSERT INTO cargo_truck(truckno,max_weight) VALUES (:truckno, :max_weight)  ';
                                        $sta=$conn->prepare($add_query);
                                        $sta-> bindValue( ":truckno", $ntruckno, PDO::PARAM_STR);
                                        $sta-> bindValue( ":max_weight", $nmaxw, PDO::PARAM_INT );
                            ...
                            if (isset($_POST["edit_btn"]))
                            {
                                ...
                                $otruckno=$_POST["otruckno"];
                                $omaxw=$_POST["omaxw"];
                                      
                                          
                                $prev_id=$_POST["previous_edit_id"];
                                $x = ($otruckno == $prev_id) ? "true" : "false" ;
                                        
                                try {
                                             
                                            $check_etruck = "SELECT * FROM cargo_truck WHERE truckno=?";
                                            $stmt_check_etruck = $dbconnection->prepare($check_etruck );
                                            $stmt_check_etruck->bind_param("s", $otruckno);
                                            $stmt_check_etruck->execute();
                                            $result_check_etruck = $stmt_check_etruck->get_result();

                                            if ($result_check_etruck->num_rows > 0 && $x == "false") {
                                                show_edit_form($otruckno,$omaxw);
                                                echo "<script>window.alert('The tuck no already exists in the table. Please enter a different tuck no.'); </script>";
                                                
                                            } else {
                                                $sql = 'UPDATE cargo_truck SET truckno=:truckno, max_weight =:max_weight WHERE truckno = :prev_id';
                                                $st=$conn->prepare($sql);                                                
                                                $st-> bindValue( ":truckno", $otruckno, PDO::PARAM_STR);
                                                $st-> bindValue( ":max_weight", $omaxw, PDO::PARAM_INT );
                                                $st-> bindValue( ":prev_id", $prev_id, PDO::PARAM_STR );
                                                $st->execute();
                         ...
                         <table class="table" cellspacing="5px" border="0"  id="table_part">
                            ...
                            <tbody>
                                ...
                                $total_rows = 0;
                                  ...
                                  include 'connect.php';                                        
                                  $sql = "SELECT COUNT(*) as total FROM cargo_truck";
                                  $result = $dbconnection->query($sql);
                                  $row = $result->fetch_assoc();
                                  $total_rows = $row['total'];
                                  $total_pages = ceil($total_rows / $records_per_page);   
                                   
                                  $sql = "SELECT * FROM cargo_truck t ORDER BY t.truckno LIMIT $start, $records_per_page";
                                  $result = $dbconnection->query($sql);                                
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                          ...
                                          <td class="d-flex flex-row justify-content-center"> 
                                              <form action="admin_cargo_truck.php" method="post" class="d-flex flex-row justify-content-evenly">
                                              <input type="hidden" value="<?php echo $truckno; ?>" name="action_id">
                        ...
                        <?php   
                          ...
                          if (isset($_POST['search_back'])){
                             echo '<script> window.location("admin_cargo_truck.php")';
                          }  
                        ...
                        <?php function add_form() { 
                          ...
                          ?>
                            ...
                            <div class="add-on" id="add_info_show">
                                <h5>Add Cargo Truck Information </h5>
                                ...
                                <tr>
                                    <td>Truck No: </td>
                                    <td><input type="text" name="ntruckno" value="" required> </td>
                                </tr>                              
                               
                                <tr>
                                    <td>Max Weight (kg): </td>
                                    <td><input type="text" name="nmaxw" value="" required> </td>
                                </tr>
                                ...
                                <a href="admin_cargo_truck.php" style="text-decoration:none;"><button type="button" class="back-btn" name="search_back"> <<< Back </button><a>
                        ...
                        <!-- edit info from -->
                        <?php function show_edit_form($etruckno,$emax_weight){ ?>
                        <!-- edit info from -->
                            <div class="add-on" id="add_info_show">
                                <h4>Edit Cargo Truck Information </h4>
                                <div style="border-bottom:1px solid black">
                                </div>

                                <form method="post" action="admin_cargo_truck.php"  enctype="multipart/form-data">      
                                <table cellspacing="10px" border="0" class="add_table">
                                <tr>
                                    <td>Truck No: <td>
                                    <td><input type="text" name="otruckno" value="<?php echo $etruckno; ?>" required> </td>
                                </tr>                                
                                <tr>
                                    <td>Max Weight (kg): <td>
                                    <td><input type="text" name="omaxw" value="<?php echo $emax_weight; ?>" required> </td>
                                </tr>
                                ...
                                <a href="admin_cargo_truck.php" style="text-decoration:none;"><button type="button" class="back-btn" name="search_back"> <<< Back </button><a>
                                <input type="hidden" name="previous_edit_id" value="<?php echo $etruckno; ?>">
                        ...
                        <?php function show_add_again($nntruck, $nnmax_w) { 
                                  include 'connect.php';                                                                   
                          ...
                          ?>
                            ...
                            <div class="add-on" id="add_info_show">
                                ...
                                <tr>
                                    <td>Truck No: </td>
                                    <td><input type="text" name="ntruckno" value="<?php echo $nntruck; ?>" required> </td>
                                </tr>                              
                                
                                <tr>
                                    <td>Max Weight(kg): </td>
                                    <td><input type="text" name="nmaxw" value="<?php echo $nnmax_w; ?>" required> </td>
                                </tr>
...
/*
function getNextTruckNo() {
    global $conn; 

    try {
        
        $sql2 = "SELECT MAX('truckno') AS max_tuck FROM cargo_truck";
        ...
        if ($result2['max_truck'] === null) {
            return 1;
        } else {
            
            return $result2['max_truck'] + 1;
        }
    } catch (PDOException $e) {
