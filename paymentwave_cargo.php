...
if (!isset($_SESSION["customer"])) {
    $_SESSION["path"]="paymentwave_cargo.php";
...
if (isset($_FILES["image"]["name"])) {
  ...
  header("Location: paymentwave_cargo.php");
...
$_SESSION["paymethod"] = htmlspecialchars("Wave-money");

if(isset($_POST["done"]))
{
$conn = connect();
$sql = "INSERT INTO get_cargo_service VALUES (:gid, :crid, :cpdate, :cid, :totalweight, :categories, :gdate, :gtime, :totalcost, :note, :sname, :semail, :stele, :rname, :remail, :rtele, :status, :screenshot, :seen, :paymethod, :message)";
...
try{
    $st = $conn->prepare($sql);
        ...
        $st->bindValue(":gid", $gid, PDO::PARAM_STR);
        $st->bindValue(":crid", $_SESSION["crid"], PDO::PARAM_INT);
        $st->bindValue(":cpdate", $_SESSION["departureDate"], PDO::PARAM_STR);
        $st->bindValue(":cid", $_SESSION["customer"], PDO::PARAM_INT);
        $st->bindValue(":totalweight", $_SESSION["weight"], PDO::PARAM_INT);
        $st->bindValue(":categories",  $_SESSION["categories"], PDO::PARAM_STR);
        $st->bindValue(":gdate", date("Y-m-d"), PDO::PARAM_STR);
        $st->bindValue(":gtime", date("H:i:s"), PDO::PARAM_STR);
        $st->bindValue(":totalcost",  $_SESSION["totalamounts"] , PDO::PARAM_INT);
        $st->bindValue(":note",  $_SESSION["note"], PDO::PARAM_STR);
        $st->bindValue(":sname", $_SESSION["sname"], PDO::PARAM_STR);
        $st->bindValue(":semail", $_SESSION["semail"], PDO::PARAM_STR);
        $st->bindValue(":stele", $_SESSION["stele"] , PDO::PARAM_STR);
        $st->bindValue(":rname",  $_SESSION["rname"], PDO::PARAM_STR);
        $st->bindValue(":remail", $_SESSION["remail"], PDO::PARAM_STR);
        $st->bindValue(":rtele", $_SESSION["rtele"], PDO::PARAM_STR);
    ...
    unset($_SESSION["stele"]);
    unset($_SESSION["rname"]);
    unset($_SESSION["remail"]);
    unset($_SESSION["rtele"]);
    unset($_SESSION["screenshot"]);
    unset($_SESSION["paymethod"]);
    unset($_SESSION["cargo_source1"]);
    unset($_SESSION["cargo_destination1"]);
    unset($_SESSION["cargo_source2"]);
    unset($_SESSION["cargo_destination2"]);
...
<header>
    ...
    <div class="progress-bar-place">
     ...
     <div class="section-container">
      <div class="section-with-background">
        <img src="images/payment/wave.png" style="width: 60px; height: 60px;"><span style="margin-left: 10px; font-size: 20px; font-weight: bold; color:white;">KBZ Pay</span>
        <div class="line"></div>
        <div class="pay">
          <p>Amount</p>
          <p><?php echo setValue("totalamounts"); ?> MMK</p>
        </div>
        <div class="qr_part" id="qr">
             <div>
                <img src="images/payment/waveqr.jpg" style="width: 250px; height:250px;">
        ...
        <div class="btns">
            ...
            <form action="paymentwave_cargo.php" method="post">
    ...
    <script type="text/javascript">
        ...
        {
            window.location.href = "cargo_page_5.php";
        }
