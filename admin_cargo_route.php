...
<html lang="en">
 ...
 if (isset($_POST["delete"]))
 {
       ...
       $c_rid = $_POST["action_id"];
       
       
       $dquery=$dbconnection->prepare("DELETE FROM cargo_route WHERE crid=$c_rid");
          ...
          <div class="col-md-12">
           ...
           <div class="bottom_work" style="background-color:#E7EEF4;">
                  <div class="header_part d-flex flex-row justify-content-between" style="border-bottom:2px solid black; padding:20px">
                    <h5>Manage Cargo Route Information</h5>
                    <form method="post" action="admin_cargo_route.php">
                ...
                <div class="whole" style="background-color:#E7EEF4;">
                    ...
                    <div class="display-info" id="table_part">
                        <?php
                            if (isset($_POST['addbtn']) || isset($_POST['edit']) || isset($_POST['search_bt']) )  {
                                ...
                                else if(isset($_POST['edit']))
                                {
                                    ...
                                    //echo $e_cid;
                                    $edit_row_query= "SELECT * FROM cargo_route WHERE crid = $e_rid";
                                    $erow=$dbconnection->query($edit_row_query);
                                    $erid=$e_rid;
                                    while($edit_row = $erow->fetch_assoc())
                                    {
                                      $csource=$edit_row["cargo_src"];
                                      $cdest=$edit_row["cargo_dest"];
                                      $ccost=$edit_row["cargo_cost"];
                                     

                                      show_edit_form($e_rid,$csource,$cdest,$ccost);
                                    }
                                }
                                else if(isset($_POST['search_bt']))
                                {
                                  $search = $_POST['search'];
                                  $sql = "SELECT * FROM cargo_route WHERE crid LIKE '%$search%' OR cargo_src LIKE '$search%' OR cargo_dest LIKE '%$search%' OR cargo_cost LIKE '%$search%' ";
                              ...
                              if (isset($_POST["edit_btn"]))
                              {
                                ...
                                $prev_rid=$_POST["previous_edit_id"];
                                  echo "<script> window.alert($prev_rid);</script>";
                                  echo "<script> window.alert($crid);</script>"; 
                                  $x = ($crid == $prev_rid) ? "true" : "false" ;
                                  try {
                                    $check_source_dest_query = "SELECT COUNT(*) AS count FROM cargo_branch WHERE branch_name IN (?, ?)";
                                    $stmt_check_source_dest = $dbconnection->prepare($check_source_dest_query);
                                    $stmt_check_source_dest->bind_param("ss", $csource, $cdest);
                                    $stmt_check_source_dest->execute();
                                    $result_check_source_dest = $stmt_check_source_dest->get_result();
                                    $row = $result_check_source_dest->fetch_assoc();

                                    if ($row['count'] < 2) {
                                        
                                        echo "<script>alert('We have no route for the provided source or destination.');</script>";
                                        display_table();
                                    } else {
                                    
                                    if($crid == $prev_rid){
                                    $sql = 'UPDATE cargo_route SET crid =:crid, cargo_src =:cargo_src,cargo_dest =:cargo_dest, cargo_cost =:cargo_cost WHERE crid = :crid';
                                  $st=$conn->prepare($sql);
                                  $st-> bindValue( ":crid", $crid, PDO::PARAM_INT );
                                  $st-> bindValue( ":cargo_src", $csource, PDO::PARAM_STR);
                                  $st-> bindValue( ":cargo_dest", $cdest, PDO::PARAM_STR);
                                  $st-> bindValue( ":cargo_cost", $ccost, PDO::PARAM_STR);

                                  $st->execute();
                                  display_table();
                                } else {
                                  $check_crid = "SELECT * FROM cargo_route WHERE crid=?";
                                          $stmt_check_crid = $dbconnection->prepare($check_crid);
                                          $stmt_check_crid->bind_param("i", $crid);
                                          $stmt_check_crid->execute();
                                          $result_check_crid = $stmt_check_crid->get_result();
                                      
                                          if ($result_check_crid->num_rows > 0 && $x="false") {
                                            show_edit_form($crid,$csource,$cdest,$ccost);
                                            echo "<script>window.alert('The route id already exists in the table. Please enter a different route.'); </script>";                                                      
                                          }
                                          else{
                                            echo "<script> window.alert('update');</script>"; 
                                            // Prepare the UPDATE statement
                                            $sql = 'UPDATE cargo_route SET crid =:crid, cargo_src =:cargo_src,cargo_dest =:cargo_dest, cargo_cost =:cargo_cost WHERE crid = :prev_rid';
                                            $st=$conn->prepare($sql);
                                            $st-> bindValue( ":crid", $crid, PDO::PARAM_INT );
                                            $st-> bindValue( ":cargo_src", $csource, PDO::PARAM_STR);
                                            $st-> bindValue( ":cargo_dest", $cdest, PDO::PARAM_STR);
                                            ...
                                            $st-> bindValue( ":prev_rid", $prev_rid, PDO::PARAM_INT );
                                            $st->execute();
                                            display_table();
                                ...
                                if(isset($_POST["add_btn"]))
                                {
                                    ...
                                    $check_source_dest_query = "SELECT COUNT(*) AS count FROM cargo_branch WHERE branch_name IN (?, ?)";
                                        ...
                                        if ($row['count'] < 2) {
                                            show_add_again($crid,$csource,$cdest,$ccost);
                                            echo "<script>alert('We have no route for the provided source or destination.');</script>";
                                        } else {
                                          try {
                                            $check_crid = "SELECT * FROM cargo_route WHERE crid=?";
                                          ...
                                          if ($result_check_crid->num_rows > 0 && $x="false") {
                                            ...
                                            $add_query = 'INSERT INTO cargo_route(crid,cargo_src,cargo_dest,cargo_cost) VALUES (:crid,:cargo_src,:cargo_dest,:cargo_cost)  ';
                                           $sta=$conn->prepare($add_query);
                                           $sta->bindValue( ":crid", $crid, PDO::PARAM_INT );
                                           $sta-> bindValue( ":cargo_src", $csource, PDO::PARAM_STR);
                                           $sta-> bindValue( ":cargo_dest", $cdest, PDO::PARAM_STR);
                                           $sta-> bindValue( ":cargo_cost", $ccost, PDO::PARAM_STR);
                                           $sta->execute();
                                           display_table();
                                          }
                                         } catch ( PDOException $e ) {
                         ...
                         <table class="table" cellspacing="5px" border="0"  id="table_part">
                            ...
                            <tbody>
                                ...
                                $total_rows = 0;
                                  ...
                                  include 'connect.php';                                        
                                  $sql = "SELECT COUNT(*) as total FROM cargo_route";
                                  $result = $dbconnection->query($sql);
                                  $row = $result->fetch_assoc();
                                  $total_rows = $row['total'];
                                  $total_pages = ceil($total_rows / $records_per_page);   
                                   
                                  $sql = "SELECT * FROM cargo_route LIMIT $start, $records_per_page";
                                  $result = $dbconnection->query($sql);                                
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            $crid = $row["crid"];
                                            $csource = $row["cargo_src"];
                                            $cdest=$row["cargo_dest"];
                                            $ccost=$row["cargo_cost"];
                                          ...
                                          <td><?php  echo $cdest; ?></td>
                                          <td><?php  echo $ccost; ?></td>
                                          
                                          <td class="d-flex flex-row justify-content-center"> 
                                              <form action="admin_cargo_route.php" method="post" class="d-flex flex-row justify-content-evenly">
                                              <input type="hidden" value="<?php echo $crid; ?>" name="action_id">
                      ...
                      <?php } ?>
                        ...
                        ?>
                         ...
                         ?>
                          ...
                          if (isset($_POST['search_back'])){
                             echo '<script> window.location("admin_cargo_route.php")';
                          } 
                        
                        ?>

                    
                        <?php function add_form($next_crid) { ?>
                        <!-- add info from -->
                            <div class="add-on" id="add_info_show">
                                <h5>Add Cargo Route Information </h5>
                                

                                <form method="post" action="admin_cargo_route.php" enctype="multipart/form-data">      
                                <table cellspacing="10px" border="0" class="add_table">
                                <tr>
                                    <td>Route Id: <td>
                                    <td><input type="text" name="crid" value="<?php echo $next_crid; ?>" required> </td>
                                </tr>
                                <tr>
                                    <td>Source: <td>
                                      <td>
                                    <select name="csource" required>
                                    <?php
                                    include 'connect.php';
                                    $sql = "SELECT branch_name FROM cargo_branch";
                                    $result = $dbconnection->query($sql);
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                ...
                                <tr>
                                    <td>Cost: <td>
                                    <td><input type="text" name="ccost" value="" required> </td>
                                </tr>
                                ...
                                <a href="admin_cargo_route.php" style="text-decoration:none;"><button type="button" class="back-btn" name="search_back"> <<< Back </button><a>
                        ...
                        <?php function show_edit_form($e_rid,$csource,$cdest,$ccost){ ?>
                        <!-- edit info from -->
                            <div class="add-on" id="add_info_show">
                                <h4>Edit Cargo Route Information </h4>
                                ...
                                <tr>
                                    <td>Route Id: <td>
                                    <td><input type="text" name="crid" value="<?php echo $e_rid; ?>" required> </td>
                                </tr>
                                <tr>
                                    <td>Source: <td>
                                    <td>
                                ...
                                <tr>
                                    <td>Cost: <td>
                                    <td><input type="text" name="ccost" value="<?php echo $ccost; ?>" required> </td>
                                </tr>
                                ...
                                <a href="admin_cargo_route.php" style="text-decoration:none;"><button type="button" class="back-btn" name="search_back"> <<< Back </button><a>
                                <input type="hidden" name="previous_edit_id" value="<?php echo $e_rid; ?>">
                                <button type="submit" name="edit_btn" value="Edit" class="btn_add" style="margin-bottom:40px;">Edit</button> </td>
                                </form>
                            </div>
                        <?php } ?>

                        <?php function show_add_again($crid,$csource,$cdest,$ccost) { ?>
                        <!-- add info from -->
                            <div class="add-on" id="add_info_show">
                                <h5>Add Cargo route Information </h5>
                                ...
                                <tr>
                                    <td>Route Id: <td>
                                    <td><input type="text" name="crid" value="<?php echo $crid; ?>" required> </td>
                                </tr>
                                <tr>
                                    <td>Source: <td>
                                    <td>
...
function getNextRouteId() {
    ...
    try {
        ...
        $sql2 = "SELECT MAX(crid) AS max_crid FROM cargo_route";
        ...
        if ($result2['max_crid'] === null) {
            return 1;
        } else {
            
            return $result2['max_crid'] + 1;
        }
    } catch (PDOException $e) {
